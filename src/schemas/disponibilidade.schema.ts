...
export const DisponibilidadeSchema =
  SchemaFactory.createForClass(Disponibilidade);
...
DisponibilidadeSchema.statics.atualizarAposReserva = async function (
  ...
  if (
    (tipo === TipoReserva.CHALE || tipo === TipoReserva.COMPLETO) &&
    quantidadeChales > 0
  ) {
    // Adicionar ou remover chalés da disponibilidade
    if (adicionar) {
      // Adicionar chalés (cancelamento de reserva)
      update.chalesDisponiveis = Math.min(
        4,
        disponibilidade.chalesDisponiveis + quantidadeChales,
      );
    } else {
      // Remover chalés (nova reserva)
      update.chalesDisponiveis = Math.max(
        0,
        disponibilidade.chalesDisponiveis - quantidadeChales,
      );
    }
